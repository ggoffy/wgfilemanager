...
$formSelect->addOption('global', \_AM_WGFILEMANAGER_PERMISSIONS_GLOBAL);
$formSelect->addOption('approve_directory', \_AM_WGFILEMANAGER_PERMISSIONS_APPROVE . ' Directory');
$formSelect->addOption('submit_directory', \_AM_WGFILEMANAGER_PERMISSIONS_SUBMIT . ' Directory');
$formSelect->addOption('view_directory', \_AM_WGFILEMANAGER_PERMISSIONS_VIEW . ' Directory');
$permTableForm->addElement($formSelect);
$permTableForm->display();
switch ($op) {
    case 'global':
    default:
        $formTitle = \_AM_WGFILEMANAGER_PERMISSIONS_GLOBAL;
        $permName = 'wgfilemanager_ac';
        $permDesc = \_AM_WGFILEMANAGER_PERMISSIONS_GLOBAL_DESC;
        $globalPerms = ['4' => \_AM_WGFILEMANAGER_PERMISSIONS_GLOBAL_4, '8' => \_AM_WGFILEMANAGER_PERMISSIONS_GLOBAL_8, '16' => \_AM_WGFILEMANAGER_PERMISSIONS_GLOBAL_16 ];
        break;
    case 'approve_directory':
        $formTitle = \_AM_WGFILEMANAGER_PERMISSIONS_APPROVE;
        $permName = 'wgfilemanager_approve_directory';
        $permDesc = \_AM_WGFILEMANAGER_PERMISSIONS_APPROVE_DESC . ' Directory';
        $handler = $helper->getHandler('directory');
        break;
    case 'submit_directory':
        $formTitle = \_AM_WGFILEMANAGER_PERMISSIONS_SUBMIT;
        $permName = 'wgfilemanager_submit_directory';
        $permDesc = \_AM_WGFILEMANAGER_PERMISSIONS_SUBMIT_DESC . ' Directory';
        $handler = $helper->getHandler('directory');
        break;
    case 'view_directory':
        $formTitle = \_AM_WGFILEMANAGER_PERMISSIONS_VIEW;
        $permName = 'wgfilemanager_view_directory';
        $permDesc = \_AM_WGFILEMANAGER_PERMISSIONS_VIEW_DESC . ' Directory';
        $handler = $helper->getHandler('directory');
        break;
}
...
unset($permform);
if (true !== $permFound) {
    \redirect_header('permissions.php', 3, \_AM_WGFILEMANAGER_NO_PERMISSIONS_SET);
    exit();
}
